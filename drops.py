...
def do_netstats(args):
    cmd = "net-stats -A -t WwQqi -i %s" % args.delta
    print(cmd)
    ob = subprocess.check_output(cmd, shell=True)
    data = json.loads(ob.decode('utf-8'), object_hook=_decode_dict)

   
    # examine vmnicd
    txpps = get_value(vmnicd, "txpps")
    rxpps = get_value(vmnicd, "rxpps")
    txeps1 = get_value(vmnicd, "txeps")
    txeps2 = get_value(vmnicd["vmnic"], "txeps")
    # txeps and rxeps are reported in two places. 
    ...
    if txeps1:
        vmnicst = vmnicst + "txeps1: %d " % txeps1
    if txeps2:
        vmnicst = vmnicst + "txeps2: %d " % txeps2
    if rxeps1:
